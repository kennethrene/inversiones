...
def ejecutar_criterio():    
    if parametros.ultimo_valor_compra != None \
        ...
        and float(parametros.valor_compra) > float(parametros.ultimo_valor_compra):
            ...
            parametros.log_operacion = f" ℹ️  Comprando por criterio rápido: Movimiento abrupto {abs(float(parametros.valor_compra) - float(parametros.ultimo_valor_compra)):.2f} supera el requerido {parametros.movimiento_abrupto[parametros.activo_actual]}"
    ...
    if parametros.ultimo_valor_venta != None \
        ...
        and float(parametros.valor_venta) < float(parametros.ultimo_valor_venta):
            ...
            parametros.log_operacion = f" ℹ️  Vendiendo por criterio rápido: Movimiento abrupto {abs(float(parametros.valor_venta) - float(parametros.ultimo_valor_venta)):.2f } supera el requerido {parametros.movimiento_abrupto[parametros.activo_actual]}"
            parametros.datos_mapeados["Criterio Apertura"] = "Criterio rápido"
            return "Vender"
