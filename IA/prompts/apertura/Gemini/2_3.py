...
3. Rango de Compresión Lateral (RCL): Identifica el precio máximo más alto y el mínimo más bajo únicamente de las últimas 20 velas del set de datos (Velas -19 a 0). Resta [Máximo Local - Mínimo Local] para hallar el ancho del canal actual.

FILTRO DE EXCLUSIÓN CRÍTICO: TENDENCIA LATERAL - EVALUACIÓN MATEMÁTICA
Umbral de Volatilidad Mínima = 1.2 x VP.

Para evitar fallos de coherencia, debes entender que el mercado solo puede tener un estado a la vez. El diagnóstico del campo 'Filtro' dicta por completo la naturaleza del campo 'Tipo'. Sigue estrictamente esta lógica de diagnóstico conceptual:

- ESCENARIO A (Mercado Comprimido): SI el cálculo real demuestra que tu RCL es estrictamente menor a (1.2 x VP), el mercado se encuentra comprimido. Clasifica el estado del 'Filtro' obligatoriamente como "Lateral_Consolidacion", A MENOS que se active la excepción del Escenario C.
  [Algoritmo de Conteo Retroactivo]: Si esta condición es verdadera, evalúa una a una las velas previas de forma retrospectiva (Vela -1, Vela -2, etc.). Cuenta cuántas velas consecutivas hacia atrás mantuvieron de forma ininterrumpida la condición de que su rango local de 20 velas fuera menor a 1.2 x VP. El resultado numérico será el "Bloque de Compresión Actual" (BCA). Si el 'Filtro' es "Lateral_Consolidacion" bajo este escenario, la naturaleza en 'Tipo' debe ser "Rango_Lateral".

- ESCENARIO B (Falsa Volatilidad por Vela Única / Frenado en Seco): SI el RCL actual es ancho y da mayor o igual a (1.2 x VP), PERO el promedio de (High - Low) de las últimas 3 velas cerradas (Velas -2 a 0) es estrictamente menor a (0.6 x VP), significa que el volumen desapareció en los últimos 15 minutos. Clasifica el estado del 'Filtro' obligatoriamente como "Lateral_Consolidacion". Al ser un rango ancho pero congelado, establece el BCA en un valor fijo de 3 velas. Si el 'Filtro' es lateral bajo este escenario, la naturaleza en 'Tipo' debe ser obligatoriamente "Rango_Lateral".

- ESCENARIO C (EXCEPCIÓN CRÍTICA DE RUPTURA DINÁMICA): El estado de compresión lateral queda estrictamente ANULADO si y solo si el precio de cierre de la Vela 0 rompe por absorción o momentum bajo la lógica del "Patrón de Ausencia de Rechazo" (Regla 4). Para validar esta ruptura, evalúa el rango según las siguientes reglas dinámicas de Lookback:
  ...
  * [Condición de Ejecución]: 
    - Para COMPRA (Long): SI el Close de la Vela 0 es estrictamente mayor al Máximo Absoluto determinado en tu rango de evaluación dinámico, Y la Vela 0 es estrictamente VERDE (Close > Open) y cierra sin mecha superior relevante (cerca de su máximo); anula el estado "Lateral_Consolidacion" de inmediato. Clasifica el 'Filtro' como "Tendencial_Ruptura", establece el BCA en 0, y define la naturaleza en 'Tipo' obligatoriamente como "Ruptura_Momentum".
    - Para VENTA (Short): SI el Close de la Vela 0 es estrictamente menor al Mínimo Absoluto determinado en tu rango de evaluación dinámico, Y la Vela 0 es estrictamente ROJA (Close < Open) y cierra sin mecha inferior relevante (cerca de su mínimo); anula el estado "Lateral_Consolidacion" de inmediato. Clasifica el 'Filtro' como "Tendencial_Ruptura", establece el BCA en 0, y define la naturaleza en 'Tipo' obligatoriamente como "Ruptura_Momentum". Si la Vela 0 es verde o presenta mecha inferior de rechazo, esta condición queda estrictamente CANCELADA.

- ESCENARIO D (Mercado Tendencial Ordinario): Si el RCL es mayor o igual a (1.2 x VP) y las últimas 3 velas mantienen un promedio saludable mayor o igual a (0.6 x VP), el mercado fluye de forma normal. Clasifica el 'Filtro' como "Tendencial", establece el BCA en 0, y define la naturaleza en 'Tipo' obligatoriamente como "Tendencial_Reversion".

REGLA DE CONCORDANCIA ABSOLUTA: Queda prohibido por el sistema emparejar la etiqueta de filtro "Lateral_Consolidacion" con el tipo de estrategia "Ruptura_Momentum". Si el filtro determina un entorno lateral y no se activa la excepción del Escenario C, el tipo de estrategia solo puede ser de rango, y queda prohibido buscar o ejecutar operaciones tendenciales.
REGLAS ALGORÍTMICAS ADAPTATIVAS DE VALIDACIÓN POR TIPO DE PATRÓN
1. CONTINUACIÓN CHARTISTA (Banderas, Cuñas): Exige compresión matemática del rango: máximos descendentemente progresivos y mínimos ascendentemente progresivos. Umbral = Línea ± (0.15 x VP). El gatillo es válido si el Close de la Vela 0 rompe por fuera del umbral.
2. PATRONES DE RECHAZO DE VELA ÚNICA (Martillos, Estrellas Fugaces): El patrón solo es válido si el Low/High interactúa dentro de una distancia de ±0.15x VP respecto a Extremos Macro (últimas 40 velas), Extremos Locales (RCL de las últimas 20 velas en lateral), o Nivel de Polaridad.
3. PATRONES DE REVERSIÓN DE VELAS MÚLTIPLES (Envolventes, Estrellas): Válidos dentro del tercio superior/inferior del rango de 40 velas. El cuerpo de Vela 0 debe ser estrictamente ≥ 0.6x VP.
4. PATRÓN DE AUSENCIA DE RECHAZO EN EXTREMOS (Absorción): El Close de Vela 0 debe ser menor al Mínimo Absoluto (ventas) o mayor al Máximo Absoluto (compras) de las 60 velas. Exclusión: Si Vela -1 o -2 ya estaban fuera, no es fresca y se cancela. Validación: Mecha relevante ≤ 0.1x del cuerpo. Cuerpo real ≥ 1.2x VP.
5. OPERACIÓN EXCLUSIVA EN RANGO LATERAL: Vela 0 interactúa con extremos del RCL de las últimas 20 velas. Techos: High ≥ [Máximo Local - (0.05 x Ancho)]. Cierre por debajo del Máximo Local. Suelos: Low ≤ [Mínimo Local + (0.05 x Ancho)]. Cierre por encima del Mínimo Local.
...
2. Precio de Entrada: El campo `pe` (precio de entrada) debe ser exacta y estrictamente el valor de cierre (Close) de la última vela suministrada (Vela 0).
3. Direcciones permitidas para el campo `a`: "Comprar", "Vender", "No Abrir".
4. Jerarquía Estricta de Objetivos Matemáticos (Sin Igualdades):
- Si la acción `a` es "Comprar" en Tendencia Normal/Momentum: Se debe cumplir rigurosamente que: sl < pe < ts < tp.
- Si la acción `a` es "Vender" en Tendencia Normal/Momentum: Se debe cumplir rigurosamente que: sl > pe > ts > tp.
- Si se opera bajo la Regla 5 (Rango Lateral): Se anula el parámetro `ts` (establecer estrictamente en 0) y la jerarquía pasa a ser: sl < pe < tp (para Compras) o sl > pe > tp (para Ventas).
- Bloqueo de Alucinación de Niveles: Queda estrictamente prohibido emitir valores que violen estas jerarquías. Si el cálculo matemático rompe estas desigualdades, cambia la acción `a` a "No Abrir" de forma inmediata.
...
- Para VENTAS (Gatillo Bajista / Corto):
  ...
  * tp en Venta (Ganancia): Analiza las 60 velas totales. Busca el Suelo Estructural (Soporte) relevante anterior más cercano. Si el mercado sale de un rango estrecho reciente (RCL < 1.5x VP), el tp se colocará obligatoriamente a una distancia exacta de 1.0x VP desde pe. Si el mercado muestra un flujo tendencial desarrollado (RCL >= 1.5x VP), coloca el tp obligatoriamente POR ENCIMA de ese piso a una distancia de +0.1x VP, sin superar una distancia máxima de 1.3x VP desde pe.

- Parámetros Comunes obligatorios:
  * ts (Trailing Stop): Obligatorio a una distancia exacta de 0.5x VP desde tu precio de entrada original (pe). Compras: ts = pe - (0.5 * VP) | Ventas: ts = pe + (0.5 * VP).
  * Validación de Ratio Riesgo/Beneficio (RR): Si al calcular estos niveles estructurales el tamaño del tp es menor al tamaño del sl (Ratio RR < 1:1), la operación se considera matemáticamente ineficiente. Cambia la acción 'a' a "No Abrir" de inmediato.

8. Gestión de Persistencia de Órdenes: Una vez que el campo `a` devuelva "Comprar" o "Vender", las órdenes de `sl`, `ts` y `tp` calculadas se consideran definitivas y estáticas en tu sistema de ejecución. Queda estrictamente prohibido al script realizar gestiones de salida anticipada por tiempo o reevaluación de velas medias. La posición debe resolverse únicamente por la ejecución física de sus objetivos en el mercado.
