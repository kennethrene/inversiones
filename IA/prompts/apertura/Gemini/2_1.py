...
- ESCENARIO A (Mercado Comprimido): SI el cálculo real demuestra que tu RCL es estrictamente menor a (1.2 x VP), el mercado se encuentra comprimido.
Clasifica el estado del 'Filtro' obligatoriamente como "Lateral_Consolidacion", A MENOS que se active la excepción del Escenario C.
...
Al ser un rango ancho pero congelado, establece el BCA en un valor fijo de 3 velas. 
Si el 'Filtro' es lateral bajo este escenario, la naturaleza en 'Tipo' debe ser obligatoriamente "Rango_Lateral".
...
momentum bajo la lógica del "Patrón de Ausencia de Rechazo" (Regla 4). Para validar esta ruptura, evalúa el rango según las siguientes reglas dinámicas de Lookback:
  ...
  * [Condición de Ejecución]: SI el Close de la Vela 0 es estrictamente mayor al Máximo Absoluto determinado en tu rango de evaluación dinámico, O estrictamente menor al 
    Mínimo Absoluto de dicho rango; anula el estado "Lateral_Consolidacion" de inmediato. Clasifica el 'Filtro' como "Tendencial_Ruptura", establece el BCA en 0, y 
    define la naturaleza en 'Tipo' obligatoriamente como "Ruptura_Momentum".
...
- ts (Trailing Stop): Obligatorio a una distancia exacta de 0.5x VP desde tu precio de entrada original (`pe`). 
  * Si es una COMPRA (Long): Se coloca por debajo de la entrada -> `ts = pe - (0.5 * VP)`
  * Si es una VENTA (Short): Se coloca por encima de la entrada -> `ts = pe + (0.5 * VP)`
