...
def calcular_metricas_vela_actual(ultima_vela, b_inferior, b_media, b_superior):
    ...
    open0  = float(ultima_vela['Open'])
    high0  = float(ultima_vela['High'])
    low0   = float(ultima_vela['Low'])
    close0 = float(ultima_vela['Close'])
    
    # 2. Cálculo de la anatomía exacta de la Vela 0
    cuerpo0    = abs(open0 - close0)
    mecha_inf0 = min(open0, close0) - low0
    mecha_sup0 = high0 - max(open0, close0)
    
    # 3. Mediciones de Clímax (Banda vs Extremos del precio)
    diferencia_bol_inf_low = b_inferior - low0
    diferencia_bol_sup_high = high0 - b_superior
